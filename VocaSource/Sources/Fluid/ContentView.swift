...
struct ContentView: View {
    ...
    @ObservedObject private var vocaCleanup = VocaCleanupMonitor.shared
    ...
    private let accessibilityRestartFlagKey = "VOCA_AccessibilityRestartPending"
    private let hasAutoRestartedForAccessibilityKey = "VOCA_HasAutoRestartedForAccessibility"
    ...
    var body: some View {
        ...
        return observed
            ...
            .onReceive(self.asr.$isRunning) { running in
                if !running { VocaAutoFinish.shared.stop() }
                else {
                    VocaAutoFinish.shared.start(audio: self.asr.audioLevelPublisher, eligible: {
                        self.asr.isRunning && self.settings.hotkeyMode == .toggle &&
                        (self.activeRecordingMode == .dictate || self.activeRecordingMode == .promptMode) &&
                        !self.isOnboardingSandboxRouteActive && !DictationPromptTestCoordinator.shared.isActive
                    }, text: { self.asr.partialTranscription }, finish: {
                        Task { await self.stopAndProcessTranscription(automaticFinish: true) }
                    })
                }
            }
            ...
            .toolbar {
                if !self.settings.shouldShowOnboarding {
                    ToolbarItemGroup(placement: .primaryAction) {
                        Button(self.vocaCleanup.slow ? "Cleanup is taking a while" : "Quick Controls", systemImage: self.vocaCleanup.slow ? "clock.badge.exclamationmark" : "slider.horizontal.3") { self.showQuickControls = true }
                            .keyboardShortcut("k", modifiers: .command).help("Quick Controls · ⌘K")
            ...
            .sheet(isPresented: self.$showQuickControls) { VocaQuickControls().appTheme(self.theme).environmentObject(self.appServices) }
            .sheet(isPresented: self.$showDestinationCheck) {
                VStack(alignment: .trailing, spacing: 12) {
                    Button("Done") { self.showDestinationCheck = false }.keyboardShortcut(.cancelAction)
                    ScrollView { VocaDestinationCheckView() }
                }.padding(20).frame(width: 570, height: 560).appTheme(self.theme).background(self.theme.palette.windowBackground)
            }
            .sheet(isPresented: self.$showVocaHelp) { VocaHelpView().appTheme(self.theme) }
            .onReceive(NotificationCenter.default.publisher(for: VocaProduct.navigation)) { notification in
                switch notification.object as? String {
                case "notes": self.navigateToApp(.changelog)
                case "feedback": self.navigateToApp(.feedback)
                case "help": self.showVocaHelp = true
                case "quickControls": self.showQuickControls = true
                case "destination": self.showDestinationCheck = true
                case "models": self.navigateToApp(.voiceEngine)
                case "accessibility": self.openAccessibilitySettings()
                case "enhancement": self.navigateToApp(.aiEnhancements)
                default: break
                }
            }
    ...
    private func openIssueReportingPage() { self.navigateToApp(.feedback) }

    @State private var showVocaHelp = false
    @State private var showQuickControls = false
    @State private var showDestinationCheck = false
    private func openHelpDocumentation() { self.showVocaHelp = true }
    ...
    private var sidebarContent: some View {
        ...
        .clipped()
        .navigationTitle(self.settingsNavigation.isPresented ? "Settings" : "VOCA")
        .tint(self.theme.palette.accent)
        .animation(self.modeTransitionAnimation, value: self.settingsNavigation.isPresented)
    }

    @State private var vocaNavigationSearch = ""
    private var vocaSidebarHeader: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack(spacing: 8) {
                ...
                TextField("Find a feature", text: self.$vocaNavigationSearch).textFieldStyle(.plain)
                    .accessibilityLabel("Find a feature")
                if !self.vocaNavigationSearch.isEmpty {
                    Button { self.vocaNavigationSearch = "" } label: { Image(systemName: "xmark.circle.fill") }
    ...
    private var appSidebarView: some View {
        VStack(spacing: 0) {
        self.vocaSidebarHeader
        List(selection: self.$selectedSidebarItem) {
            Section {
                self.sidebarNavigationLink(.welcome, title: "Getting Started", systemImage: "square.grid.2x2")
            }
            Section {
        ...
        .animation(self.accessibilityReduceMotion ? nil : .easeInOut(duration: 0.16), value: self.selectedSidebarItem)
        ...
        .background(self.theme.palette.sidebarBackground.opacity(0.72))
    ...
    private var settingsSidebarView: some View {
        VStack(spacing: 0) {
            ...
            .help("Back to VOCA")
            .accessibilityLabel("Back to VOCA")
    ...
    private var helpEntryButton: some View {
        ...
        .accessibilityHint("Opens the VOCA help guide")
    }
    ...
    private func vocaPageTitle(_ title: String) -> String {
        ["Voice Engine": "Speech Models", "AI Providers": "Text Enhancement", "Cleanup Styles": "Writing Styles", "Custom Dictionary": "Dictionary", "Getting Started": "Overview", "Change logs": "What's New", "Stats": "Usage", "File Transcription": "Transcribe a File"][title] ?? title
    }
    private func vocaMatches(_ title: String) -> Bool {
        let aliases = ["Voice Engine": "speech models whisper parakeet nemotron apple cohere", "AI Providers": "api openai claude anthropic gemini groq local ollama", "Cleanup Styles": "fixer enhancement prompts formatting", "Custom Dictionary": "vocabulary pronunciation replacements", "File Transcription": "audio video import"]
        return self.vocaNavigationSearch.isEmpty || (title + " " + self.vocaPageTitle(title) + " " + (aliases[title] ?? "")).localizedCaseInsensitiveContains(self.vocaNavigationSearch)
    }
    @ViewBuilder private func sidebarSectionHeader(_ title: String) -> some View {
        let groups = ["Configure": ["Voice Engine", "AI Providers", "Cleanup Styles", "Custom Dictionary"], "Use": ["Command Mode", "File Transcription"], "Activity": ["History", "Stats"], "Help": ["Getting Started", "Change logs", "Feedback"]]
        if (groups[title] ?? [title]).contains(where: { self.vocaMatches($0) }) {
            Text(title)
                .font(self.theme.typography.sidebarSection).foregroundStyle(.secondary).textCase(nil)
                .padding(.top, self.theme.metrics.spacing.sm).padding(.bottom, self.theme.metrics.spacing.xs)
        }
    }

    @ViewBuilder private func sidebarNavigationLink(_ item: SidebarItem, title: String, systemImage: String) -> some View {
        let isSelected = self.selectedSidebarItem == item
        if self.vocaMatches(title) {
        NavigationLink(value: item) {
            HStack(spacing: self.theme.metrics.spacing.sm) {
                Image(nsImage: SidebarSymbolCache.image(named: systemImage))
                    ...
                    .resizable()
                    .scaledToFit()
                    .foregroundStyle(isSelected ? Color.white : self.theme.palette.secondaryText)
                    .frame(width: 15, height: 15)
                    .padding(5)
                    .accessibilityHidden(true)

                Text(self.vocaPageTitle(title))
                    .foregroundStyle(isSelected ? Color.white : Color.primary)
            }
            .font(self.theme.typography.sidebarItem)
    ...
    private var detailView: some View {
        ZStack {
            VocaWindowBackground()

            // Preserve the app destination so Back never waits on expensive detail initialization.
            self.appDetailContent
                .id(self.selectedSidebarItem)
                .transition(self.accessibilityReduceMotion ? .identity : .opacity.combined(with: .offset(y: 4)))
                .animation(self.accessibilityReduceMotion ? nil : .easeOut(duration: 0.18), value: self.selectedSidebarItem)
                .opacity(self.settingsNavigation.isPresented ? 0 : 1)
            ...
            if self.settingsNavigation.isPresented {
                self.preferencesView
                    .animation(self.accessibilityReduceMotion ? nil : .easeOut(duration: 0.16), value: self.settingsNavigation.selectedSection)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if self.asr.micStatus == .notDetermined {
                    ...
                    self.instructionStep(number: "2", text: "Find **VOCA** in the microphone list")
                    self.instructionStep(number: "3", text: "Toggle **VOCA ON** to allow access")
                }
            }
            .padding(.leading, 4)
        }
    ...
    private func stopAndProcessTranscription(route: DictationOutputRoute = .normal, automaticFinish: Bool = false) async {
        VocaAutoFinish.shared.stop()
        let pipelineID = UUID().uuidString
        await DebugLogger.$pipelineID.withValue(pipelineID) {
            await self.processStoppedTranscription(route: route, pipelineID: pipelineID, automaticFinish: automaticFinish)
        }
    }

    private func processStoppedTranscription(route: DictationOutputRoute, pipelineID: String, automaticFinish: Bool) async {
        ...
        var originalWordsRequested = false
        ...
        let spokenSendParse = SpokenSendParser.parse(
            ...
            enabled: route == .normal && !automaticFinish && self.settings.spokenSendEnabled
        )
        ...
        if shouldUseAI {
            ...
            let streamHandler: PrivateAIStreamHandler = { chunk in
                Task { @MainActor in
                    guard VocaCleanupMonitor.shared.acceptsPreview(cleanupID) else { return }
            ...
            do {
                ...
                let result = try await VocaCleanupMonitor.shared.run(id: cleanupID,
                    label: postProcessingModelInfo.model ?? "AI cleanup") {
                try await self.processTextWithAIMetrics(
                ...
                originalWordsRequested = (error as? VocaCleanupError) == .skipped
                aiFallbackReason = error.localizedDescription
                aiFallbackNotificationError = (error as? VocaCleanupError) == nil ? DictationAIFailurePresentationPolicy.notificationMessage(for: error) : nil
        ...
        let restoreBypassDestination = originalWordsRequested && self.recordingFocusTarget != nil
            && self.recordingFocusTarget?.pid != ProcessInfo.processInfo.processIdentifier
        let isFluidFrontmost = frontmostApp?.bundleIdentifier == Bundle.main.bundleIdentifier && !restoreBypassDestination
        ...
        let destinationStillActive = self.recordingFocusTarget.map { TypingService.isExactFocusTargetActive($0) } ?? false
        let guardedDelivery = automaticFinish || shouldUseAI
        let shouldTypeExternally = shouldPersistOutputs && !isFluidFrontmost
            && (!guardedDelivery || destinationStillActive || restoreBypassDestination)
        if guardedDelivery && !shouldTypeExternally {
            let recoveryID = UUID()
            VocaInsertionRecovery.shared.begin(recoveryID)
            VocaInsertionRecovery.shared.unavailable(text: finalText,
                message: "Your words are ready. The destination changed while VOCA was working, so nothing was inserted.", id: recoveryID)
        }
        ...
        if shouldTypeExternally {
            let typingTarget = restoreBypassDestination
                ? (pid: self.recordingFocusTarget?.pid, shouldRestoreOriginalFocus: true)
            ...
            if typingTarget.shouldRestoreOriginalFocus && !automaticFinish {
            ...
            if spokenSendAllowed {
                ...
                self.asr.typeOutputPlanToActiveField(
                    ...
                    requiredFocusTarget: guardedDelivery ? self.recordingFocusTarget : nil,
...
extension ContentView {
    ...
    private func positionWindowBesideSystemSettings(requestID: UUID) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) {
            ...
            guard let window = NSApp.windows.first(where: { $0.isVisible && $0.title == "VOCA" }) ?? NSApp.keyWindow else {
                return
            }
    ...
    private func cancelAccessibilityPermissionFlow() {
        ...
        (NSApp.windows.first { $0.isVisible && $0.title == "VOCA" } ?? NSApp.keyWindow)?
    ...
    private var draggableAccessibilityAppURL: URL {
        ...
        let installedURL = URL(fileURLWithPath: "/Applications/VOCA.app")
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("One small permission. Words everywhere.").font(.headline)
                    Text("Drag VOCA into the Accessibility list, then turn it on.")
                        .font(.callout).foregroundStyle(.secondary)
                }
                Spacer()
                Button(action: onClose) { Image(systemName: "xmark.circle.fill").font(.title3).foregroundStyle(.secondary) }
                    .buttonStyle(.plain).accessibilityLabel("Close permission guide")
            }
            HStack(spacing: 14) {
                Image(nsImage: appIcon).resizable().frame(width: 44, height: 44)
                VStack(alignment: .leading, spacing: 3) {
                    Text(appName).font(.headline)
                    Text("Drag this app card ↑").font(.caption).foregroundStyle(.secondary)
                }
                Spacer()
                Image(systemName: "hand.draw").font(.title2).foregroundStyle(.blue)
            }.padding(12).background(.white.opacity(0.06), in: RoundedRectangle(cornerRadius: 16))
                .onDrag { NSItemProvider(object: appURL as NSURL) }
                .accessibilityLabel("Drag this VOCA build into the Accessibility list")
            HStack {
                Button("Show in Finder") { NSWorkspace.shared.activateFileViewerSelecting([appURL]) }
                Spacer()
                Button("Back to VOCA", action: onReturnToApp)
            }.font(.callout)
...
struct CardAppearAnimation: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .offset(y: self.reduceMotion || self.appear ? 0 : 4)
            .opacity(self.appear ? 1.0 : 0)
            .animation(self.reduceMotion ? nil : .easeOut(duration: 0.18), value: self.appear)
    }
}

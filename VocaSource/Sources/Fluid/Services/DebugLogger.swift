...
final nonisolated class DebugLogger: @unchecked Sendable {
    ...
    static func diagnosticsEnabled(in defaults: UserDefaults = .standard) -> Bool {
        defaults.bool(forKey: "voca.diagnostics.enabled")
    }

    static func redactCredentials(_ message: String) -> String {
        var value = message
        for pattern in [#"(?i)Bearer\s+[A-Za-z0-9_./+=-]+"#,
                        #"sk-[A-Za-z0-9_-]{12,}"#,
                        #"(?i)(?:api[_-]?key|x-api-key)[\"'\s:=]+[A-Za-z0-9_./+=-]{8,}"#] {
            value = value.replacingOccurrences(of: pattern, with: "[credential redacted]", options: .regularExpression)
        }
        return value
    }
    ...
    private func write(_ message: String, level: LogLevel, source: String, pipelineID: String?) {
        ...
        let formattedLine = self.formatLogLine(
            ...
            level: level,
            source: source,
            message: Self.redactCredentials(message),
            pipelineID: pipelineID
        )

        // Detailed content is persisted only after explicit diagnostic opt-in.

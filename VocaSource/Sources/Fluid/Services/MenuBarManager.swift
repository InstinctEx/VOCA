...
final class MenuBarManager: NSObject, ObservableObject, NSMenuDelegate {
    ...
    private func updateMenuBarIcon() {
        ...
        if let image = NSImage(systemSymbolName: "waveform", accessibilityDescription: "VOCA") {
    ...
    private func buildMenuStructure() {
        ...
        // Open Main Window
        let openItem = NSMenuItem(title: "Open VOCA", action: #selector(openMainWindow), keyEquivalent: "")
        openItem.target = self
        menu.addItem(openItem)

        let quickItem = NSMenuItem(title: "Quick Controls…", action: #selector(openVocaQuickControls), keyEquivalent: "k")
        quickItem.target = self
        quickItem.keyEquivalentModifierMask = [.command]
    ...
    @objc private func rollbackToPreviousVersion(_ sender: Any?) {
        ...
        confirm.informativeText = "This will restore the backup and relaunch VOCA."
        ...
        Task { @MainActor in
            do {
                ...
                success.informativeText = "Rolled back to \(availableVersion). VOCA will relaunch shortly."
    ...
    @objc private func openMainWindow() {
        ...
        if let window = mainWindows.first {
            ...
            window.animationBehavior = NSWorkspace.shared.accessibilityDisplayShouldReduceMotion ? .none : .documentWindow
    ...
    @objc private func openVocaQuickControls() {
        self.openMainWindow()
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { VocaProduct.navigate("quickControls") }
    }

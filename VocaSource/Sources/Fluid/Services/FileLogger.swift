...
final nonisolated class FileLogger: @unchecked Sendable {
    ...
    private init() {
        ...
        self.logDirectory = baseDirectory.appendingPathComponent("Logs/VOCA", isDirectory: true)
        self.logFileURL = self.logDirectory.appendingPathComponent("VOCA.log", isDirectory: false)
        self.backupLogURL = self.logDirectory.appendingPathComponent("VOCA.log.1", isDirectory: false)
    ...
    private func appendUnlocked(line: String) {
        ...
        let line = DebugLogger.redactCredentials(line)
        self.createLogDirectoryIfNeeded()
        self.rotateIfNeeded(force: false)

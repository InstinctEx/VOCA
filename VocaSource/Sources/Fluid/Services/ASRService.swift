...
final class ASRService: ObservableObject {
    ...
    private var benchmarkCancellationRequested = false
    ...
    func start(
        ...
        guard !self.isBenchmarking else {
            self.errorTitle = "Model check in progress"
            self.errorMessage = "Wait for the short model timing check to finish, then dictate again."
            self.showError = true
            return .failed
        }
    ...
    func cancelVocaSpeechBenchmark() { self.benchmarkCancellationRequested = true }
    private func checkVocaBenchmarkCancellation() throws {
        if self.benchmarkCancellationRequested { throw CancellationError() }
    }

    /// Benchmarks only the selected installed local provider; no formatting, history, or insertion.
    func runVocaSpeechBenchmark() async throws -> String {
        let model = SettingsStore.shared.selectedSpeechModel
        guard !self.isRunningOrStarting, !self.isBenchmarking, !self.isStoppingFinalTranscription, !self.recordingBufferHandoffGate.isActive,
              !self.hasActiveModelDownload, !self.hasActiveModelPreparation,
              model.isInstalled, model.provider != .apple else {
            throw NSError(domain: "VOCA.Benchmark", code: 1, userInfo: [NSLocalizedDescriptionKey: "Finish dictation and choose an installed local model first."])
        }
        self.isBenchmarking = true
        self.benchmarkCancellationRequested = false
        defer { self.isBenchmarking = false }
        guard let url = Bundle.main.url(forResource: "voca_benchmark", withExtension: "wav") else {
            throw NSError(domain: "VOCA.Benchmark", code: 2, userInfo: [NSLocalizedDescriptionKey: "The benchmark sample is missing. Reinstall this VOCA build."])
        }
        let reader = try LocalAPIAudioDecoder.ChunkReader(fileURL: url)
        let samples = try await reader.nextSamples()
        try await self.ensureAsrReady(source: .settings)
        let provider = self.transcriptionProvider
        let duration = Double(samples.count) / 16_000
        let elapsed = try await self.transcriptionExecutor.run { [provider] in
            try await self.checkVocaBenchmarkCancellation()
            _ = try await provider.transcribeFinal(samples) // Warm run; model loading is excluded.
            var times: [Double] = []
            for _ in 0..<3 {
                try await self.checkVocaBenchmarkCancellation()
                let start = ProcessInfo.processInfo.systemUptime
                _ = try await provider.transcribeFinal(samples)
                times.append(ProcessInfo.processInfo.systemUptime - start)
            }
            try await self.checkVocaBenchmarkCancellation()
            return times.sorted()[1]
        }
        return "\(model.displayName): \(String(format: "%.2f", elapsed))s for \(String(format: "%.1f", duration))s of speech · \(String(format: "%.1f", duration / max(elapsed, 0.001)))× real time. Median of 3 warm runs; this measures speed, not accuracy."
    }
...
private extension ASRService {
    ...
    static var streamingRecoveryMessage: String {
        "Speech recognition took too long to finish. This recording could not be transcribed. " +
            "Wait for the model to recover, or restart VOCA before recording again."
    }

...
enum SimpleUpdateError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .vocaChannelNotConfigured: return "VOCA’s release channel is not configured for this preview."
...
final class SimpleUpdater {
    ...
    func checkForUpdate(
        ...
        guard Bundle.main.bundleIdentifier?.hasPrefix("com.voca.") != true else { throw SimpleUpdateError.vocaChannelNotConfigured }
    ...
    private func showUpdateInstallStatus(version: String) {
        ...
        panel.title = "Installing VOCA \(version)"
        ...
        let title = NSTextField(labelWithString: "Installing VOCA \(version)")
        title.frame = NSRect(x: 92, y: 76, width: 304, height: 24)
        title.font = .systemFont(ofSize: 16, weight: .semibold)
        content.addSubview(title)

        let detail = NSTextField(wrappingLabelWithString: "Downloading the update. VOCA will restart automatically.")
    ...
    private func rollbackRootDirectory() -> URL {
        ...
        return support
            .appendingPathComponent("VOCA-Fluid", isDirectory: true)

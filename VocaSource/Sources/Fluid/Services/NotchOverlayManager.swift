...
final class NotchOverlayManager {
    ...
    var isNotchContracting: Bool { self.state == .hiding && self.notch != nil }
    ...
    private func showInternal(audioLevelPublisher: AnyPublisher<CGFloat, Never>, mode: OverlayMode) {
        ...
        if SettingsStore.shared.overlayPosition != .top {
    ...
    private func showNotchOverlay(audioLevelPublisher: AnyPublisher<CGFloat, Never>, mode: OverlayMode, screen: NSScreen?) {
        ...
        newNotch.transitionConfiguration = .init(
            openingAnimation: .snappy(duration: NSWorkspace.shared.accessibilityDisplayShouldReduceMotion ? 0 : 0.28),
            closingAnimation: .snappy(duration: NSWorkspace.shared.accessibilityDisplayShouldReduceMotion ? 0 : 0.24),
            conversionAnimation: .snappy(duration: NSWorkspace.shared.accessibilityDisplayShouldReduceMotion ? 0 : 0.24),
    ...
    /// Completes without blocking text insertion. Bottom panels disappear
    /// immediately; the non-activating notch contracts in place.
    func hideImmediately() {
        // Completion must not delay insertion. Let the top notch contract in
        // its non-activating panel while the caller inserts the transcript.
        if self.notch != nil && !self.isBottomOverlayVisible {
            self.hide()
            return
        }
        let startedAt = ProcessInfo.processInfo.systemUptime
    ...
    private func performHideAndWait(generation currentGeneration: UInt64) async -> RecordingOverlayHideOutcome {
        ...
        let closingNotch = self.notch
        self.notchAnimationTask?.cancel()
        self.notchPresentationTask?.cancel()
        await closingNotch?.hide()
        guard self.generation == currentGeneration else { return .superseded }
        self.notch = nil
    ...
    private func preferredPresentationScreen() -> NSScreen {
        // The physical camera notch is the anchor even when the pointer is on
        // an external display. Closed-lid setups retain the floating fallback.
        if SettingsStore.shared.overlayPosition == .top,
           let notchedScreen = NSScreen.screens.first(where: { self.supportsCompactPresentation(on: $0) }) {

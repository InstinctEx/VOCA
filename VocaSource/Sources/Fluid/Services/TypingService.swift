            ...
            DebugLogger.$pipelineID.withValue(pipelineID) {
                ...
                defer {
                    ...
                    Task { @MainActor in
                        ...
                        if !completedOutcome.didInsert || postInsertionKey != nil {
                            VocaInsertionRecovery.shared.unavailable(text: text,
                                message: postInsertionKey != nil ? "This dictation included a send action. Automatic undo is unavailable; your words are kept here."
                                    : "The last insertion could not be delivered. Your words are kept here to recover.", id: recoveryID)
                        }
                ...
                if let requiredFocusTarget,
                   (requiredFocusTarget.isSecureTextField || !Self.isExactFocusTargetActive(requiredFocusTarget)) {
                    outcome = .rejected
                    return
                }
                ...
                if hasTextToInsert {
                    ...
                    let recoveryTarget = Self.captureSystemFocusTarget()
                    let recoveryBefore = recoveryTarget.flatMap { target in
                        (preferredTargetPID == nil || target.pid == preferredTargetPID) ? VocaDestination.snapshot(target) : nil
                    }
                    let inserted = self.insertTextInstantly(text, preferredTargetPID: preferredTargetPID)
                    if inserted, postInsertionKey == nil {
                        DispatchQueue.global(qos: .utility).async {
                            let expected = recoveryBefore.map { ($0.value as NSString).replacingCharacters(in: $0.selection, with: text) }
                            var verified = false
                            for _ in 0..<8 {
                                if let before = recoveryBefore, let expected,
                                   let value = VocaDestination.attribute(before.target.element, kAXValueAttribute) as? String,
                                   VocaDestination.exactPrefix(value, expected) {
                                    verified = true
                                    break
                                }
                                usleep(75_000)
                            }
                            let didVerify = verified
                            Task { @MainActor in
                                VocaInsertionRecovery.shared.record(before: recoveryBefore, inserted: text, verified: didVerify, id: recoveryID)
                            }
                        }
                    }
                    self.bench(

...
private struct MicrophoneChangeOverlayView: View {
    ...
    private var explanation: String {
        ...
        if self.notice.presentation == .startupSelection {
            return "VOCA will try this microphone first when you dictate."
        }
        if self.notice.presentation == .selectionChange {
            return "VOCA selected this microphone for capture."
        }
        return "VOCA confirmed this microphone is capturing audio."
    }
}
...
private struct AutomaticDictionaryCorrectionOverlayView: View {
    ...
    private var choiceContent: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Save only this correction, or teach VOCA other pronunciations.")
                .font(.system(size: 11))
                .foregroundStyle(.white.opacity(0.58))
                .lineLimit(1)
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 9) {
                ...
                if self.session.isReady {
                    Label("VOCA got it right 3 times in a row.", systemImage: "checkmark.circle.fill")
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        self.trainingInstruction(
                            number: 2,
                            text: "Say the word, then pause. VOCA captures it and listens again."
                        )

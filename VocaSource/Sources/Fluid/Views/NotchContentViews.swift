...
struct NotchCompactTrailingView: View {
    ...
    var body: some View {
        HStack(spacing: self.showsSpokenSendIndicator ? 4 : 0) {
            if cleanup.slow {
                Button { cleanup.useOriginal() } label: { Image(systemName: "clock.badge.exclamationmark").foregroundStyle(.orange) }
                    .buttonStyle(.plain).help("Cleanup is taking a while. Use original words now.").accessibilityLabel("Use original words now")
            } else if autoFinish.countdown != nil { VocaPauseCountdown() }
            else {
            CompactNotchWaveformView(
                audioPublisher: self.audioPublisher,
                color: self.contentState.mode == .dictation ? Color.white.opacity(0.92) : self.contentState.mode.notchColor,
...
struct NotchCompactBottomView: View {
    ...
    var body: some View {
        ...
        .frame(width: self.previewWidth, height: self.shouldShowPreview ? self.previewHeight : 0, alignment: .leading)
        .padding(.horizontal, self.shouldShowPreview ? 10 : 0)
        .padding(.bottom, self.shouldShowPreview ? 8 : 0)
        .clipped()

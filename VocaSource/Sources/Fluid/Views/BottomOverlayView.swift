    ...
    func show(audioPublisher: AnyPublisher<CGFloat, Never>, mode: OverlayMode) {
        ...
        if self.caretTarget != nil {
            self.caretTrackingTimer = Timer.scheduledTimer(withTimeInterval: 0.4, repeats: true) { [weak self] timer in
                Task { @MainActor [weak self] in
                    guard let self, NotchContentState.shared.isBottomOverlayPresented, !self.isHideInProgress else {
                        timer.invalidate()
                        return
                    }
...
private final class BottomOverlayHostingView: NSHostingView<BottomOverlayView> {
    override func hitTest(_ point: NSPoint) -> NSView? {
        if SettingsStore.shared.overlaySize == .pill || SettingsStore.shared.overlayPosition == .caret {
...
struct BottomOverlayView: View {
    ...
    private var layout: LayoutConstants {
        LayoutConstants.get(for: self.settings.overlayPosition == .caret ? .pill : self.settings.overlaySize)
    }
    ...
    private var isPillSize: Bool {
        self.settings.overlaySize == .pill || self.settings.overlayPosition == .caret
    }
    ...
    @ObservedObject private var vocaAutoFinish = VocaAutoFinish.shared
    private var vocaHasTranscript: Bool {
        self.settings.overlayPosition != .caret && self.settings.enableStreamingPreview && !self.contentState.cachedPreviewText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }
    private var vocaCompactPill: some View {
        VStack(spacing: 9) {
            HStack(spacing: 12) {
                self.leadingAppContextView.accessibilityLabel("Destination application")
                Group {
                    if self.vocaCleanup.slow {
                        Button { self.vocaCleanup.useOriginal() } label: { Image(systemName: "clock.badge.exclamationmark").foregroundStyle(.orange) }
                            .buttonStyle(.plain).help("Cleanup is taking a while. Use original words now.").accessibilityLabel("Use original words now")
                    } else if self.vocaAutoFinish.countdown != nil {
                        VocaPauseCountdown()
                    } else if self.contentState.isProcessing {
                        ProgressView().controlSize(.small).tint(.white).accessibilityLabel("Processing dictation")
                    } else {
                        BottomWaveformView(color: .white.opacity(0.9), layout: self.layout, visibleBarCount: nil)
                    }
                }.frame(width: self.layout.waveformWidth, height: 28)
            }
            .padding(.horizontal, 16).padding(.vertical, 10)
            .vocaGlass(cornerRadius: 26)
            .colorScheme(.dark)
            if self.vocaHasTranscript {
                Text(self.contentState.cachedPreviewText)
                    .font(.system(size: 14, weight: .medium)).foregroundStyle(.primary)
                    .lineLimit(2).truncationMode(.head)
                    .padding(.horizontal, 18).padding(.vertical, 12)
                    .frame(maxWidth: 440, alignment: .leading)
                    .vocaGlass(cornerRadius: 17)
            }
        }
        .animation(self.reduceMotion ? nil : .easeOut(duration: 0.16), value: self.vocaHasTranscript)
    }

    var body: some View {
        Group {
        if self.isPillSize { self.vocaCompactPill }
        else {
        ...
        .frame(
            width: self.isPillSize ? (self.vocaHasTranscript ? 440 : 148) : (self.layout.usesFixedCanvas ? self.layout.overlayWidth : self.layout.containerWidth),
        ...
        .onChange(of: self.contentState.cachedPreviewText) { _, _ in
            if self.isPillSize { BottomOverlayWindowController.shared.refreshSizeForContent() }

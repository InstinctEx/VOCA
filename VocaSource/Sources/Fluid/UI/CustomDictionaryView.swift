...
struct CustomDictionaryView: View {
    ...
    @State private var vocaDictionaryTab = "Phrases"
    @State private var phraseSearch = ""
    @State private var showPhraseComposer = false
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: self.theme.metrics.spacing.xl) {
                ...
                Picker("Dictionary section", selection: self.$vocaDictionaryTab) {
                    Text("Phrases").tag("Phrases")
                    Text("Names & Vocabulary").tag("Vocabulary")
                    Text("Punctuation").tag("Punctuation")
                }.pickerStyle(.segmented).labelsHidden()
                    .disabled(self.isTrainingRecording || self.isTrainingProcessing)
                VStack(alignment: .leading, spacing: 20) {
                    switch self.vocaDictionaryTab {
                    case "Vocabulary": self.aiPostProcessingSection
                    case "Punctuation": self.punctuationDictionarySection
                    default:
                        self.phraseLibrary
                        if self.showPhraseComposer { self.trainReplacementSection }
                    }
                }
            }
        ...
        .onAppear {
            ...
            if self.entries.isEmpty { self.showPhraseComposer = true }
            self.loadBoostTerms()
    ...
    private var pageHeader: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            HStack {
                ...
                Spacer()
                Menu {
                    Button("Import Dictionary", systemImage: "square.and.arrow.down", action: self.importDictionary)
                    Button("Export Dictionary", systemImage: "square.and.arrow.up", action: self.exportDictionary)
            ...
            VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text("Show VOCA the right spelling, by voice or by typing.")
    ...
    private var dictionaryComposerModeSegmented: some View {
        Picker("Add a phrase using", selection: Binding(get: { self.composerMode }, set: { self.selectComposerMode($0) })) {
            Text("Speak it").tag(DictionaryComposerMode.train)
            Text("Type it").tag(DictionaryComposerMode.manual)
    ...
    private var trainReplacementComposer: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            TextField("Type the correct text, e.g. VOCA", text: self.$trainingReplacement)
    ...
    private var manualTriggerField: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            Text("When VOCA hears")
                .font(self.theme.typography.captionStrong)

            TextField("vo ka, vokah", text: self.$manualTriggerDraft)
                .dictionaryInputChrome()
    ...
    private var manualReplacementField: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            Text("Change it to")
                .font(self.theme.typography.captionStrong)
            TextField("VOCA", text: self.$manualReplacement)
                .dictionaryInputChrome()
                .onSubmit { self.addManualReplacementIfValid() }
    ...
    private var trainingRecorderPanel: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.md) {
            Text("Teach VOCA your pronunciation")
            ...
            if self.trainingAlreadyCorrectWithoutReplacement {
                ...
                Label(
                    self.activePronunciationMatching
                        ...
                        : "VOCA recognized \(self.trainingTargetReference) 3 times in a row.",
                ...
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    self.trainingInstruction(
                        number: 3,
                        text: "Say \(self.trainingTargetReference) naturally, then pause. VOCA records and listens again automatically."
                    )
                    self.trainingInstruction(
                        number: 4,
                        text: self.activePronunciationMatching
                            ? "Repeat 3 times to teach VOCA how your voice sounds."
                            : "Repeat the phrase until all samples are learned."
                    )
                }
            }
        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Phrase Library").font(.title3.weight(.semibold))
                    Text(self.entries.isEmpty ? "A place for words that are uniquely yours." : "\(self.entries.count) saved \(self.entries.count == 1 ? "phrase" : "phrases")")
                        .font(.caption).foregroundStyle(.secondary)
                }
                Spacer()
                Button(self.showPhraseComposer ? "Hide editor" : "Add phrase", systemImage: self.showPhraseComposer ? "minus" : "plus") {
                    withAnimation(.easeInOut(duration: 0.18)) { self.showPhraseComposer.toggle() }
                }.fluidButton(.primary, size: .small).disabled(self.isTrainingRecording || self.isTrainingProcessing)
            }
            if !self.entries.isEmpty {
                TextField("Find a phrase or spelling", text: self.$phraseSearch).textFieldStyle(.roundedBorder)
                let matches = self.entries.filter { self.phraseSearch.isEmpty || $0.replacement.localizedCaseInsensitiveContains(self.phraseSearch) || $0.triggers.contains(where: { $0.localizedCaseInsensitiveContains(self.phraseSearch) }) }
                if matches.isEmpty { Text("No matching phrases.").font(.callout).foregroundStyle(.secondary) }
                ForEach(matches) { entry in
                    HStack(alignment: .center, spacing: 14) {
                        VStack(alignment: .leading, spacing: 5) {
                            Text(CustomDictionaryManualEntry.replacementDisplayText(entry.replacement)).font(.body.weight(.medium)).textSelection(.enabled)
                            Text(entry.triggers.joined(separator: " · ")).font(.callout).foregroundStyle(.secondary).lineLimit(2)
                        }.frame(maxWidth: .infinity, alignment: .leading)
                        Button("Edit") { self.editingEntry = entry }.buttonStyle(.borderless)
                        Menu { Button("Delete phrase", role: .destructive) { self.deleteEntry(entry) } }
                            label: { Image(systemName: "ellipsis") }
                            .menuStyle(.borderlessButton).fixedSize().accessibilityLabel("Actions for \(entry.replacement)")
                    }.padding(.vertical, 8)
                    if entry.id != matches.last?.id { Divider() }
                }
            } else {
                Label("Add a spelling, phrase, or spoken shortcut below.", systemImage: "text.badge.plus")
        ...
        ThemedCard(style: .standard, hoverEffect: false) {
            HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("Words and phrases VOCA will correct automatically.")
                        .font(self.theme.typography.caption)
                        .foregroundStyle(self.theme.palette.secondaryText)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
        ...
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
            HStack(alignment: .top, spacing: self.theme.metrics.spacing.md) {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("VOCA automatically corrects these words and phrases.")
                        .font(self.theme.typography.caption)
                        .foregroundStyle(self.theme.palette.secondaryText)
                }
    ...
    private var boostWordEditor: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.md) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text("Word or Phrase")
                    .font(self.theme.typography.captionStrong)
                TextField("VOCA", text: self.$boostTermText)
                ...
                VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                    ...
                    HStack(alignment: .top, spacing: self.theme.metrics.spacing.md) {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Try Saying")
                                .font(self.theme.typography.captionStrong)
                            Text("Examples of what VOCA will type.")
                                .font(self.theme.typography.caption)
                                .foregroundStyle(self.theme.palette.secondaryText)
        ...
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            ...
            if self.punctuationRules.isEmpty {
                self.dictionaryEmptyState(
                    ...
                    detail: "Add what you say and what VOCA should type."
                )
            } else {
...
private struct VoiceMatchingSettingsRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            Picker("Voice matching", selection: Binding(get: { self.isEnabled }, set: {
                self.isEnabled = $0
                self.onChange($0)
            })) {
                Text("Standard").tag(false)
                Text("Match pronunciation · Preview").tag(true).disabled(!self.isAdvancedAvailable)
            }.pickerStyle(.menu).disabled(self.isDisabled)
            ...
            if self.isEnabled {
                HStack(alignment: .top, spacing: 8) {
                    Image(systemName: "flask.fill")
                        .foregroundStyle(self.theme.palette.accent)
                    Text("Research Preview: Compares how your voice sounds instead of only the words VOCA hears. Results may vary.")
                        .font(self.theme.typography.caption)
                        .foregroundStyle(self.theme.palette.secondaryText)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    static func readinessCaption(
        ...
        if isReady {
            return usesVoiceMatching
                ? "Ready. VOCA learned how \(target) sounds in your voice."
                : "Ready. VOCA got \(target) right 3 times in a row."
        }
        return usesVoiceMatching
            ? "Say \(target) 3 times to unlock Add Replacement."
            : "Keep trying until VOCA gets \(target) right 3 times in a row."
    }
}
...
private struct DictionaryTrainingReadinessRing: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            Label(self.isReady ? "Voice learned" : "Learning your voice", systemImage: self.isReady ? "checkmark.circle.fill" : "waveform")
                .font(.caption.weight(.medium))
            HStack(spacing: 5) {
                ForEach(0..<max(1, self.total), id: \.self) { index in
                    Capsule().fill(index < self.progress ? self.theme.palette.accent : self.theme.palette.cardBorder)
                        .frame(width: 28, height: 5)
                }
            }
            Text(self.isReady ? "Ready to save" : "\(self.progress) of \(self.total) samples")
                .font(.caption2).foregroundStyle(.secondary)
        }.frame(width: 145, alignment: .leading)
            .accessibilityElement(children: .ignore).accessibilityLabel("Voice learning")
            .accessibilityValue("\(self.progress) of \(self.total) samples")
    }
}
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("This is what will appear in the final transcription.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                TextField("VOCA", text: self.$replacement)
                    .dictionaryInputChrome()

...
extension AIEnhancementSettingsView {
    ...
    var aiConfigurationCard: some View {
        VStack(alignment: .leading, spacing: 28) {
            ...
            case .providers: self.providerConfigurationContent
            case .advancedPrompts: self.promptsStepContent
            }
        }
    }
    ...
    private var aiSetupHeader: some View {
        ...
        return VocaPageHeader(
            title: isProviders ? "Text Enhancement" : "Writing Styles",
            subtitle: isProviders ? "A finishing touch for your words. Connect a service or choose a local model." : "Your words, with just the right tone. Create a style for any moment.",
            symbol: isProviders ? "sparkles" : "textformat"
        )
    }
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 0) {
                ForEach(filteredItems) { item in
                    self.providerCard(item).id(item.id)
    ...
    private func providerCard(_ item: ProviderItem) -> some View {
        ...
        .background(isExpanded ? Color.fluidGreen.opacity(0.025) : Color.clear)
    }
    ...
    private func providerStatus(for item: ProviderItem) -> (text: String, color: Color, icon: String) {
        ...
        case .unknown:
            return ("Set up connection", .secondary, "")
        }
    }
    ...
    var appleIntelligenceBadge: some View {
        HStack(spacing: 8) {
            ...
            Text("•").foregroundStyle(self.theme.palette.secondaryText)
    ...
    var appleIntelligenceModelRow: some View {
        HStack(spacing: 12) {
            ...
            Text("System Language Model").foregroundStyle(self.theme.palette.secondaryText).font(.system(.body))
            Spacer()
        }
    }

...
struct StatsView: View {
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 24) {
                VocaPageHeader(title: "Usage", subtitle: "A little less typing. A little more time for you.", symbol: "chart.bar.xaxis")
                self.timeSavedHero
                ...
                DisclosureGroup("Milestones & personal records") {
                    VStack(spacing: 16) { self.milestonesCard; self.recordsCard }.padding(.top, 16)
                }.font(.callout.weight(.medium)).padding(20).vocaContentSurface()
                DisclosureGroup("More insights") { self.insightsCard.padding(.top, 16) }
                    .font(.callout.weight(.medium)).padding(20).vocaContentSurface()
    ...
    private var timeSavedHero: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            ViewThatFits(in: .horizontal) {
                ...
                VStack(alignment: .leading, spacing: 12) { self.trendLabel; self.typingSpeedButton }
            }
        }.frame(maxWidth: .infinity, alignment: .leading).padding(26).vocaContentSurface()
            .popover(isPresented: self.$showWPMEditor) { self.wpmEditorPopover }
    }

    private var trendLabel: some View {
        let days = self.historyStore.dailyWordCounts(days: self.chartDays * 2)
        let recent = days.suffix(self.chartDays).reduce(0) { $0 + $1.words }
        let previous = days.prefix(self.chartDays).reduce(0) { $0 + $1.words }
        return VStack(alignment: .leading, spacing: 4) {
            Text("\(self.formatNumber(recent)) words in the last \(self.chartDays) days").font(.callout.weight(.medium))
            Text(previous > 0 ? String(format: "%+.0f%% vs. the previous %d days", Double(recent - previous) / Double(previous) * 100, self.chartDays) : "Your trend begins here.")
                .font(.caption).foregroundStyle(.secondary)
        }
    }

    private var typingSpeedButton: some View {
        Button("Estimate · \(self.settings.userTypingWPM) WPM", systemImage: "slider.horizontal.3") {
    ...
    private var streakCard: some View {
        StatCard(title: "CURRENT STREAK", icon: "flame.fill") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Best: \(self.historyStore.bestStreak) \(self.historyStore.bestStreak == 1 ? "day" : "days")")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
            }
        }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if maxWords == 0 {
                    ...
                    HStack {
                        ...
                        Text("across \(activeDays) active \(activeDays == 1 ? "day" : "days")")
                            .font(.system(size: 11))
                            .foregroundStyle(.secondary)

    ...
    var body: some View {
        GeometryReader { geometry in
            if geometry.size.width < 720 {
                if self.compactDetailPresented, let entry = self.selectedEntry {
                    VStack(alignment: .leading, spacing: 0) {
                        Button("All recordings", systemImage: "chevron.left") { self.compactDetailPresented = false }
                            .buttonStyle(.borderless).padding(16)
                        self.entryDetailView(entry).frame(maxWidth: .infinity)
                    }
                } else { self.historyList }
            } else {
                HSplitView {
                    self.historyList.frame(minWidth: 240, idealWidth: 290, maxWidth: 340)
                    Group {
                        if let entry = self.selectedEntry { self.entryDetailView(entry) }
                        else { self.noSelectionView }
                    }.frame(minWidth: 340)
                }
            }
        }
        ...
        .alert("Example Exported", isPresented: self.$showReportConfirmation) {
            Button("OK", role: .cancel) {}
        } message: {
            Text("Your example was saved on this Mac. You can share the file yourself.")
        }
    ...
    private func entryRow(_ entry: TranscriptionHistoryEntry) -> some View {
        ...
        return Button {
            withAnimation(.easeInOut(duration: 0.15)) {
                ...
                self.compactDetailPresented = true
            }
        } label: {
        ...
        .contextMenu {
            ...
            Button {
                ...
                Label("Export Example…", systemImage: "hand.thumbsup.slash")
            }
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack(spacing: 8) {
                        ...
                        .buttonStyle(.bordered)
                        .controlSize(.small)
                        .help("Review and export this example locally")
...
private struct TranscriptionFeedbackReportSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading, spacing: 4) {
                Text("Export a dictation example")
                    .font(.system(size: 18, weight: .semibold))
                Text("Review the text below before saving. The file may contain personal information; nothing is sent automatically.")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
            }
            ...
            HStack {
                ...
                Button {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text(self.isSending ? "Exporting…" : "Export Example…")
                    }
                }
    ...
    private func sendReport() async {
        ...
        do {
            let exported = try await TranscriptionFeedbackReporter.export(payload)
            self.isSending = false
            if exported { self.onSent() }
        } catch {

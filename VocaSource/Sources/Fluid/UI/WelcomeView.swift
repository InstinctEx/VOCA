...
struct WelcomeView: View {
    ...
    private var modelReady: Bool { self.asr.isAsrReady || self.asr.modelsExistOnDisk }
    private var ready: Bool { self.modelReady && self.asr.micStatus == .authorized && self.accessibilityEnabled }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 28) {
                ...
                VStack(spacing: 0) {
                    HStack(spacing: 9) {
                        Image(systemName: self.ready ? "checkmark.circle.fill" : "slider.horizontal.3")
                            .foregroundStyle(self.ready ? Color.green : Color.fluidGreen)
                        Text(self.ready ? "Ready when you are" : "Make yourself at home")
                            .font(.system(size: 15, weight: .semibold))
                            .contentTransition(.opacity)
                            .animation(self.reduceMotion ? nil : .easeInOut(duration: 0.16), value: self.ready)
                        Spacer()
                        Menu {
                            Button("Check permissions & insertion") { VocaProduct.navigate("destination") }
                            Button("Quick Controls") { VocaProduct.navigate("quickControls") }
                            Divider()
                        ...
                        .menuStyle(.borderlessButton).frame(width: 24).help("Setup options")
                    }.padding(20)
                    Divider().padding(.horizontal, 20)
                    self.readinessRow("Speech model", detail: self.modelReady ? self.settings.selectedSpeechModel.humanReadableName : "Choose a model to get started", symbol: "waveform", complete: self.modelReady) {
                        self.selectedSidebarItem = .voiceEngine
                    }
                    self.readinessRow("Microphone", detail: self.asr.micStatus == .authorized ? "Connected to your voice" : "Allow microphone access", symbol: "mic", complete: self.asr.micStatus == .authorized) {
                        if self.asr.micStatus == .notDetermined { self.asr.requestMicAccess() }
                        else if self.asr.micStatus != .authorized { self.asr.openSystemSettingsForMic() }
                    }
                ...
                LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 12), count: 3), spacing: 12) {
                    ...
                    self.destination("Writing Styles", detail: "Sound like yourself", symbol: "textformat", item: .cleanupStyles)
                    self.destination("Text Enhancement", detail: "Add a finishing touch", symbol: "sparkles", item: .aiEnhancements)
                }
                }
                VStack(alignment: .leading, spacing: 16) {
                    HStack {
                        VocaSectionHeading(title: "Try a few words", detail: self.asr.isRunning ? "Listening…" : "A space to practice")
                        Button {
                            ...
                            Label(self.asr.isRunning ? "Finish" : "Try Dictation", systemImage: self.asr.isRunning ? "stop.fill" : "mic.fill")
                                .foregroundStyle(.white)
                        }
                        .fluidButton(.primary, size: .small, isRecording: self.asr.isRunning)
                        .disabled(!self.asr.isAsrReady && !self.asr.isRunning)
                    }
                    VocaPracticeEditor(
                        text: Binding(get: { self.asr.finalText }, set: { self.asr.finalText = $0 }),
                        placeholder: self.asr.isRunning ? "Listening to you…" : "Your words will appear here.",
                        focus: self.isTranscriptionFocused,
                        placeholderColor: self.theme.palette.tertiaryText
                    ).frame(height: 126)
                    if !self.asr.finalText.isEmpty {
                        HStack {
                            ...
                            Button("Clear") { self.asr.finalText = "" }
                            Spacer()
                            Text("\(self.asr.finalText.count) characters").font(.caption).foregroundStyle(self.theme.palette.secondaryText)
    ...
    private func readinessRow(_ title: String, detail: String, symbol: String, complete: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 14) {
                Image(systemName: symbol).font(.system(size: 16)).foregroundStyle(self.theme.palette.secondaryText).frame(width: 24)
                Text(title).font(.system(size: 13, weight: .medium))
                Spacer()
                Text(detail).font(.system(size: 12)).foregroundStyle(self.theme.palette.secondaryText).lineLimit(1)
                Image(systemName: complete ? "checkmark" : "chevron.right")
                    .font(.system(size: 11, weight: .semibold)).foregroundStyle(complete ? Color.green : Color.secondary)
                            ...
                            VStack(spacing: 14) {
                                self.permissionRow(
                                    ...
                                    subtitle: self.isMicrophoneReady
                                        ? "Choose the microphone you want VOCA to use."
                                ...
                                if !self.isAccessibilityReady {
                                    Text("Already enabled it? VOCA will update when macOS confirms access.")
                                        .font(.system(size: 12, weight: .medium))
                                        .foregroundStyle(Color.white.opacity(0.42))
                                        .padding(.top, 2)
                                }
        ...
        VStack(spacing: 0) {
            HStack(spacing: 14) {
                ...
                if self.devices.isEmpty {
                    ...
                    .frame(width: 248)
                    .tint(.white)
                    .accessibilityHint("Moves the selected microphone to first in VOCA priority")
                }
            }
            .padding(.horizontal, 18)
            .frame(height: 62)

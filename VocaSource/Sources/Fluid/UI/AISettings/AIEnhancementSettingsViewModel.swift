...
final class AIEnhancementSettingsViewModel: ObservableObject {
    ...
    private func probeKeychainAccess() -> KeychainAccessCheckResult {
        let service = "com.voca.fluid.provider-api-keys"
    ...
    private func keychainPermissionExplanation(for status: OSStatus) -> String {
        var message = "VOCA stores provider API keys securely in your macOS Keychain but does not currently have permission to access it."
        if let detail = SecCopyErrorMessageString(status, nil) as String? {
            message += "\n\nmacOS reported: \(detail) (\(status))"
        }
        message += "\n\nClick \"Always Allow\" when the Keychain prompt appears, or open Keychain Access > login > Passwords, locate the VOCA entry, and grant access."
        return message
    }

    private func keychainPersistenceExplanation(for error: Error) -> String {
        var message = "VOCA could not save the API key to your macOS Keychain, so this provider was not verified."
    ...
    func presentKeychainAccessAlert(message: String) {
        let msg = message.isEmpty
            ? "VOCA stores provider API keys securely in your macOS Keychain. Please grant access by choosing \"Always Allow\" when prompted."
            : message
    ...
    func addCurrentAppPromptBinding(for mode: SettingsStore.PromptMode) {
        guard let target = self.resolveBindingTargetApp() else {
            self.appPromptBindingErrorMessage = "Could not detect a target app. Focus another app window (outside VOCA) and try again."
            DebugLogger.shared.info(

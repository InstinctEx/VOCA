...
private struct VocaNativeGlassButtonStyle: PrimitiveButtonStyle {
    var prominent = false
    ...
    func makeBody(configuration: Configuration) -> some View {
        if self.prominent {
            ...
            Button(role: configuration.role, action: configuration.trigger) {
                configuration.label.foregroundStyle(self.destructive ? Color.red : Color.primary)
...
extension View {
    ...
    func fluidButton(
        ...
        if #available(macOS 26.0, *) {
            switch role {
            case .primary, .accent:
                self.buttonStyle(VocaNativeGlassButtonStyle(prominent: true))
                    .tint(isRecording ? .red : Color.fluidGreen)
                    .controlSize(size == .large ? .large : .regular)
            case .destructive:
                self.buttonStyle(VocaNativeGlassButtonStyle(destructive: true))
            case .inline:
                self.buttonStyle(.borderless)
            default:
                self.buttonStyle(VocaNativeGlassButtonStyle())
                    .controlSize(size == .large ? .large : .regular)
            }
        } else {
            switch role {
            case .primary, .accent:
                self.buttonStyle(.borderedProminent).tint(isRecording ? .red : Color.fluidGreen)
            case .destructive:

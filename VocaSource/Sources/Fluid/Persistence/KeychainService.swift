...
final class KeychainService {
    ...
    private let service = "com.voca.fluid.provider-api-keys"
    ...
    /// Interactive access is only requested by the explicit Unlock action, away from the main thread.
    func requestAccess() throws {
        _ = try self.loadStoredKeys(forceRefresh: true, allowAuthentication: true)
    }
    ...
    func legacyProviderEntries() throws -> [String: String] {
        ...
        case errSecSuccess:
            ...
            for attributes in attributesArray {
                ...
                dataQuery[kSecUseAuthenticationUI as String] = kSecUseAuthenticationUIFail
    ...
    private func loadStoredKeys(forceRefresh: Bool = false, allowAuthentication: Bool = false) throws -> [String: String] {
        if !forceRefresh, case let .loaded(keys) = self.cachedState() { return keys }

        if allowAuthentication {
            self.ioLock.lock()
        } else {
            guard self.ioLock.try() else { throw KeychainServiceError.unhandled(errSecInteractionNotAllowed) }
        }
        defer { self.ioLock.unlock() }
        ...
        let keys = try self.readStoredKeys(allowAuthentication: allowAuthentication)
        self.setCachedKeys(keys)
        return keys
    }

    private func readStoredKeys(allowAuthentication: Bool = false) throws -> [String: String] {

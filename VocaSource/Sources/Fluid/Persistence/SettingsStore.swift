...
final class SettingsStore: ObservableObject {
    ...
    private(set) var launchAtStartupStatusMessage =
        "VOCA reflects the actual macOS login item state. Unsigned or development builds may fail to enable this."

    private init() {
        if !self.defaults.bool(forKey: "VOCAGlassDesignV1") {
            self.defaults.set("Purple", forKey: Keys.accentColorOption)
            self.defaults.set("pill", forKey: Keys.overlaySize)
            self.defaults.set("bottom", forKey: Keys.overlayPosition)
            self.defaults.set(true, forKey: "VOCAGlassDesignV1")
        }
        // Adopt the notch presentation once; later user choices remain persistent.
        if !self.defaults.bool(forKey: "VOCANotchDesignV1") {
            self.defaults.set("top", forKey: Keys.overlayPosition)
            self.defaults.set("minimal", forKey: Keys.notchPresentationMode)
            self.defaults.set(true, forKey: "VOCANotchDesignV1")
        }
        // One-time adoption of VOCA's native blue accent. Later choices persist.
        if !self.defaults.bool(forKey: "VOCANativeBlueV1") {
            self.defaults.set("Blue", forKey: Keys.accentColorOption)
            self.defaults.set(true, forKey: "VOCANativeBlueV1")
        }
    ...
    private func ensureDebugLoggingDefaults() {
        if self.defaults.object(forKey: Keys.enableDebugLogs) == nil {
            self.defaults.set(false, forKey: Keys.enableDebugLogs)
        }
    }
    ...
    enum OverlayPosition: String, CaseIterable, Codable {
        ...
        case caret // Beside the insertion point

        var displayName: String {
            switch self {
            case .top: return "MacBook Notch"
            case .bottom: return "Bottom of Screen"
            case .caret: return "Near the Typing Cursor"
            }
        }
    }
    ...
    var overlaySize: OverlaySize {
        get {
            guard let raw = self.defaults.string(forKey: Keys.overlaySize),
                ...
                return .pill // Compact VOCA waveform
            }
            return size
        }
        set {
    ...
    enum AccentColorOption: String, CaseIterable, Identifiable, Codable {
        ...
        var hex: String {
            ...
            case .purple: return "#9674E8"
            case .orange: return "#F59E0B"
            }
        }
        var color: Color {
            // System blue adapts to the current appearance and Increase Contrast.
            self == .blue ? Color(nsColor: .systemBlue) : (Color(hex: self.hex) ?? Color(nsColor: .systemBlue))
        }
    }
    ...
    enum TranscriptionStartSound: String, CaseIterable, Identifiable, Codable {
        ...
        var displayName: String {
            ...
            case .fluidSfx0: return "VOCA SFX 0"
            case .fluidSfx1: return "VOCA SFX 1"
            case .fluidSfx2: return "VOCA SFX 2"
            case .fluidSfx3: return "VOCA SFX 3"
            case .fluidSfx4: return "VOCA SFX 4"
            }
        }
    ...
    private func migrateProviderAPIKeysIfNeeded() {
        // Never mistake denied access for an empty keychain or discard a migration source before saving.
        do {
            ...
            if !legacyDefaults.isEmpty || !legacyKeychain.isEmpty {
    ...
    /// For example: ["fluid voice", "fluid boys"] -> "VOCA"
    ...
    enum SpeechModel: String, CaseIterable, Identifiable, Codable {
        ...
        /// Optional badge text for the card (e.g., "VOCA Pick")
        var badgeText: String? {
            switch self {
            case .parakeetTDT: return "VOCA Pick"
            case .parakeetTDTv2: return "VOCA Pick"
...
private extension SettingsStore {
    ...
    enum Keys {
        ...
        static let enableDebugLogs = "voca.diagnostics.enabled"

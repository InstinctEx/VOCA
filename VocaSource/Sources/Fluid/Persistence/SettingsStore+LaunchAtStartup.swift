...
extension SettingsStore {
        ...
        do {
            ...
            if self.launchAtStartupEnabled != enabled {
                let mismatchMessage = enabled
                    ? "macOS did not enable VOCA in Login Items. Check System Settings > General > Login Items."
                    : "macOS still shows VOCA in Login Items. Check System Settings > General > Login Items."
    ...
    private func launchAtStartupFailureMessage(for error: Error, enabling: Bool) -> String {
        ...
        {
            return "VOCA could not \(action) launch at startup. This build may not be signed correctly for macOS Login Items."
        }

        if lowercasedDescription.contains("approval") ||
            lowercasedDescription.contains("authorize")
        {
            return "macOS needs approval before VOCA can \(action) launch at startup. Check System Settings > General > Login Items."
        }

        return "VOCA could not \(action) launch at startup. macOS reported: \(nsError.localizedDescription)"
    }
    ...
    private var compatibilityLoginItemName: String {
        Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "VOCA"
    }
...
private enum LaunchAtStartupSystemState {
    ...
    var message: String {
        switch self {
        case .enabled:
            return "VOCA reflects the actual macOS login item state."
        case .disabled:
            return "VOCA reflects the actual macOS login item state. Unsigned or development builds may fail to enable this."
        case .requiresApproval:
            return "macOS requires approval for VOCA in Login Items before launch at startup becomes active."
        }
    }
}
#endif

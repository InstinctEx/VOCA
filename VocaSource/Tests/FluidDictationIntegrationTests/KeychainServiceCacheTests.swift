...
final class KeychainServiceCacheTests: XCTestCase {
    ...
    func testDeniedAggregateReadCannotOverwriteSavedKeys() {
        struct AccessDenied: Error {}
        var didSave = false
        let service = KeychainService(testingLoad: { throw AccessDenied() }, testingSave: { _ in didSave = true })
        XCTAssertThrowsError(try service.storeAllKeys(["openai": "new"]))
        XCTAssertFalse(didSave)
    }

    func testExplicitUnlockRefreshesTheCache() throws {
        var storage = ["openai": "old"]
        let service = KeychainService(testingLoad: { storage }, testingSave: { _ in })
        _ = try service.fetchAllKeys()
        storage = ["openai": "new"]
        try service.requestAccess()
        XCTAssertEqual(try service.fetchKey(for: "openai"), "new")
    }
}

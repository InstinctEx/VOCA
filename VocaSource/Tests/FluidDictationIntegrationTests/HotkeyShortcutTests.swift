...
final class HotkeyShortcutTests: XCTestCase {
    ...
    func testMicrophoneChangeAlertsSupportProductionAndDebugAppsOnly() {
        XCTAssertTrue(MicrophoneChangeOverlayController.supportsAlerts(bundleIdentifier: "com.voca.fluid"))
        XCTAssertTrue(MicrophoneChangeOverlayController.supportsAlerts(bundleIdentifier: "com.voca.fluid.debug"))
    ...
    @MainActor
    func testCompletedNotchContractsWithoutBlockingInsertion() async throws {
        let settings = SettingsStore.shared
        let originalPosition = settings.overlayPosition
        let originalMode = settings.notchPresentationMode
        defer { settings.overlayPosition = originalPosition; settings.notchPresentationMode = originalMode }
        settings.overlayPosition = .top
        settings.notchPresentationMode = .minimal
        let manager = NotchOverlayManager.shared
        manager.show(audioLevelPublisher: Just(CGFloat.zero).eraseToAnyPublisher(), mode: .dictation)
        try await Task.sleep(for: .milliseconds(600))
        XCTAssertTrue(manager.isOverlayVisible)
        manager.hideImmediately()
        await Task.yield()
        XCTAssertTrue(manager.isNotchContracting, "Keep the panel alive for the closing shape animation")
        let outcome = await manager.hideAndWait()
        XCTAssertEqual(outcome, .hidden)
        XCTAssertFalse(manager.isNotchContracting)
        XCTAssertFalse(manager.isAnyNotchVisible)
    }

    @MainActor
    func testNewDictationSupersedesContractingNotch() async throws {
        let settings = SettingsStore.shared
        let originalPosition = settings.overlayPosition
        defer { settings.overlayPosition = originalPosition }
        settings.overlayPosition = .top
        let manager = NotchOverlayManager.shared
        let audio = Just(CGFloat.zero).eraseToAnyPublisher()
        manager.show(audioLevelPublisher: audio, mode: .dictation)
        try await Task.sleep(for: .milliseconds(600))
        let hideTask = Task { @MainActor in await manager.hideAndWait() }
        await Task.yield()
        manager.show(audioLevelPublisher: audio, mode: .dictation)
        let oldOutcome = await hideTask.value
        XCTAssertEqual(oldOutcome, .superseded)
        try await Task.sleep(for: .milliseconds(600))
        XCTAssertTrue(manager.isOverlayVisible, "The old closing task must not dismiss the new recording")
